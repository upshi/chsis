...
$(function() {
	/*导航颜色*/
	$("#navi_hospitalManager_doctor").addClass('active');
	
	//绑定点击添加医生按钮事件
	$('.btn-add-doctor').on('click', function(){
		var hospitalUuid = $('#hospitalUuid').val();
		$.ajax({
			url : "department/getDepartment/" + hospitalUuid,
			type : "GET" ,
			cache : false , 
			dataType : "json" ,
...
//删除医生操作
function deleteDoctor(uuid,url) {
	$.confirm({
	...
	$.ajax({
			...
			success : function(data) {
				if(data.result == 'success') {
					...
					$('#doctor-department').html(data.doctor.department.name);
